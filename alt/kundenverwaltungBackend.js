...
const server = http.createServer(async (req, res) => {
    ...
    // �berpr�fe, ob die Anfrage eine 'OPTIONS'-Anfrage ist (CORS Preflight-Anfrage)
    if (req.method === 'OPTIONS') {
        res.writeHead(204); // Sende eine leere Antwort f�r die Preflight-Anfrage
        res.end();
        return;
    }
    ...
    // �berpr�fe, ob der Pfad '/get-data' ist und die Methode 'GET' ist
    if (path === '/get-data' && req.method === 'GET') {
        ...
        try {
            ...
            // Sende die Daten als JSON zur�ck
            res.writeHead(200);
            ...
            if (conn) conn.release(); // Verbindung zur�ckgeben
        }
    } else {
        // 404 Not Found f�r unbekannte Routen
